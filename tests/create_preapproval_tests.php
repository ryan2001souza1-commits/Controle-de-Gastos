 ...
 *
 * Cobre:
 * - plano Pro com dados corretos
 * - plano Premium com dados corretos
 * - payer_email correto no payload
 * - external_reference correto no payload
 * - preapproval_plan_id correto no payload
 * - back_url correto no payload
 * - POST /preapproval (verifica metodo e URL)
 * - redirecionamento usando init_point da resposta
 * - ausencia de init_point na resposta
 ...
 * - back_url invalido
 *
 * Usa mock FakeCurlMpService que intercepta curl_exec via propriedade $curlQueue.
 */
...
$r = $s->createPreapproval('', 'user@test.com', 'user_1_pro', 'https://example.com/return');
...
$r = $s->createPreapproval('plan_pro_xyz', 'usuario@ex.com', 'user_1_pro', 'https://example.com/return');
...
assert_test(!isset($s->lastCall['postfields']['card_token_id']), 'Pro: card_token_id NAO enviado');
assert_test(!isset($s->lastCall['postfields']['auto_recurring']), 'Pro: auto_recurring NAO enviado');
assert_test(!isset($s->lastCall['postfields']['status']) || $s->lastCall['postfields']['status'] === 'pending', 'Pro: status=pending (nao authorized)');
...
$r = $s->createPreapproval('plan_premium_xyz', 'premium@ex.com', 'user_5_premium', 'https://example.com/return');
...
echo "\n--- Redirecionamento com init_point da resposta ---\n";
...
$r = $s->createPreapproval('plan_pro_xyz', 'user@test.com', 'user_1_pro', 'https://example.com/return');
$redirectUrl = $r['init_point'] ?? '';
assert_test($redirectUrl !== '', 'init_point presente na resposta -> usado para redirect');
assert_test(filter_var($redirectUrl, FILTER_VALIDATE_URL), 'init_point e URL valida');
...
$r = $s->createPreapproval('plan_pro_xyz', 'user@test.com', 'user_1_pro', 'https://example.com/return');
assert_test($r['ok'] === false, 'sem init_point -> ok=false');
assert_test($r['error'] === 'missing_init_point', 'sem init_point -> error=missing_init_point');
...
$r = $s->createPreapproval('valid_plan_xyz', 'user@test.com', 'user_1_pro', 'https://example.com/return');
...
$s->createPreapproval('plan_pro_xyz', 'user@test.com', 'user_1_pro', 'https://example.com/return');
$payload = $s->lastCall['postfields'];

$forbidden = ['card_token_id', 'auto_recurring', 'reason', 'payment_method_id'];
