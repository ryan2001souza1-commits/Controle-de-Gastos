...
<div style="display:flex;justify-content:flex-end;margin-bottom:12px"><button type="button" class="btn btn-primary" style="background:#059669;border-color:#059669" onclick="openGoalModal()"><?= render_icon('plus',13) ?> Nova meta</button></div>

<section class="metric-strip">
    <article class="metric-card"><div class="metric-card-icon" style="background:#ecfdf5;color:#10b981"><?= render_icon('target',18) ?></div><div class="metric-card-body"><div class="metric-card-label">Total de metas</div><div class="metric-card-value"><?= count($goals) ?></div><div class="text-xs" style="color:#64748b"><?= $completed ?> concluídas</div></div></article>
    <article class="metric-card"><div class="metric-card-icon" style="background:#eff6ff;color:#3b82f6"><?= render_icon('pie',18) ?></div><div class="metric-card-body"><div class="metric-card-label">Em andamento</div><div class="metric-card-value"><?= $inProgress ?></div><div class="text-xs" style="color:#64748b"><?= count($goals)>0 ? round($inProgress/count($goals)*100) : 0 ?>% do total</div></div></article>
    <article class="metric-card"><div class="metric-card-icon" style="background:#f5f3ff;color:#7c3aed"><?= render_icon('check',18) ?></div><div class="metric-card-body"><div class="metric-card-label">Concluídas</div><div class="metric-card-value"><?= $completed ?></div><div class="text-xs" style="color:#64748b"><?= count($goals)>0 ? round($completed/count($goals)*100) : 0 ?>% do total</div></div></article>
    <article class="metric-card"><div class="metric-card-icon" style="background:#fffbeb;color:#d97706"><?= render_icon('wallet',18) ?></div><div class="metric-card-body"><div class="metric-card-label">Total poupado</div><div class="metric-card-value" style="color:#0f172a">R$ <?= number_format($totalSaved,2,',','.') ?></div><div class="text-xs" style="color:#64748b">de R$ <?= number_format($totalObj,2,',','.') ?></div></div></article>
</section>
...
<div style="display:grid;grid-template-columns:1fr 340px;gap:16px;align-items:start">
    ...
    <div style="display:flex;flex-direction:column;gap:16px">
        <section class="panel"><header class="panel-header"><div class="panel-title" style="font-size:14px">Progresso geral</div></header><div class="panel-body" style="text-align:center">
            <div style="position:relative;width:140px;height:140px;margin:0 auto"><svg width="140" height="140" viewBox="0 0 120 120" style="transform:rotate(-90deg)"><circle cx="60" cy="60" r="54" fill="none" stroke="#e2e8f0" stroke-width="12"/><circle cx="60" cy="60" r="54" fill="none" stroke="#10b981" stroke-width="12" stroke-linecap="round" stroke-dasharray="<?= $progressPct/100*339 ?> 339"/></svg><div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center"><div style="font-size:24px;font-weight:700;color:#0f172a"><?= $progressPct ?>%</div><div style="font-size:11px;color:#64748b">do total</div></div></div>
            <div style="margin-top:12px"><div style="color:#10b981;font-weight:700">R$ <?= number_format($totalSaved,2,',','.') ?></div><div style="font-size:11px;color:#64748b">de R$ <?= number_format($totalObj,2,',','.') ?></div></div>
        </div></section>
            ...
            <div class="modal-body">
                <form id="goalForm" action="/index.php?action=store_goal" method="POST">
                    <?= csrf_field() ?>
                        ...
                        <div class="form-group"><label>Descrição (opcional)</label><textarea name="description" rows="2" placeholder="Detalhes da meta"></textarea></div>
                    </div>
                </form>
            </div>
            <footer class="modal-footer"><button type="button" class="btn btn-ghost" onclick="closeGoalModal()">Cancelar</button><button type="submit" form="goalForm" class="btn btn-primary" style="background:#059669">Salvar meta</button></footer>
        </div>
    </div>
    <script>
    function openGoalModal(){document.getElementById('goalModal').style.display='flex';document.body.style.overflow='hidden'}
    function closeGoalModal(){document.getElementById('goalModal').style.display='none';document.body.style.overflow=''}
